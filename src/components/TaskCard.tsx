...
const TaskCard = (props: TaskCardProps) => {
  ...
  return (
    <div
      ...
      {...attributes}
      key={key}
      className="flex justify-between cursor-grab rounded-lg bg-nutral-50 p-4 shadow-sm hover:shadow-md"
      style={style}
    >
      <div>
        <h3 className="font-medium text-nutral-700">{task.title}</h3>
        <p className="mt-2 text-sm text-nutral-500">{task.description}</p>
      </div>
      <div
